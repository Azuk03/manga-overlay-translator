...
- `detectAndInpaintTiled(blob, naturalW, naturalH, img)` — webtoon dài (mục 5.5.1).
...
### 5.5.1 Webtoon dài — cắt lát (content.js:477, `detectAndInpaintTiled`)
...
2. `detectAndInpaintTiled()` gọi backend **tuần tự** từng lát, cộng offset `y` của lát vào bbox trả về.
...
### 5.8.1 Split-phase pipeline — Pha A (GPU) chồng lấn Pha B (mạng) (2026-09-05)

**Vấn đề:** `translateAndRenderImage` (mục 5.8, nay đã thay bằng `runPhaseA`/`runPhaseB`) gọi backend 1 lần duy nhất cho mỗi ảnh, gồm detect+OCR+**dịch GPT**+inpaint+render — GPT nằm XEN GIỮA, nên toàn bộ thời gian chờ mạng (GPT có thể mất vài giây tới hơn nửa phút khi backend phải thử lại — mục 8) diễn ra TRONG LÚC ảnh đang giữ khoá executor GPU duy nhất (`CONCURRENCY:1`, mục 5.12) — chặn luôn ảnh kế tiếp, dù GPU đang rảnh.

**Giải pháp:** tách 1 request thành 2 pha ĐỘC LẬP:
- **Pha A (GPU)** — `runPhaseA()` (content.js:1506) gọi ĐÚNG endpoint cũ (`/translate/with-form/json/stream`, không cần backend mới), nhưng luôn ép `translator: 'none'` (`motPhaseAConfig()`, `extension/content-script/phase-payload.js`) — vẫn detect+OCR+inpaint thật (`lama_mpe`/1024) nhưng KHÔNG gọi GPT. Trả về toàn bộ vùng + `background` đã inpaint; `dst` chưa có (bằng `src`).
- **Pha B (mạng)** — `runPhaseB()` (content.js:1578) gọi endpoint MỚI `/translate/texts` (`patches/main.py`) — chỉ nhận mảng chuỗi, dịch qua GPT/Gemini/DeepL, **không đụng GPU, không giữ khoá executor** (đọc đúng nguyên văn docstring `TranslateTextsRequest`: *"endpoint nay chay trong TIEN TRINH SERVER va KHONG di qua executor, nen khong giu khoa GPU"*). Merge ghép-biên (mục 5.7) dời tới SAU bước này (so sánh độ dài `dst`, chỉ có nghĩa sau khi dịch xong). `OverlayRenderer.render()` và `registerRenderedRegion` cũng nằm ở đây.
- **`Queue._handoff`** (`motCreateHandoff(2)`, content.js:1677) — bộ đệm chặn-trên-2 giữa 2 pha: A-worker chạy Pha A cho ảnh kế tiếp NGAY (không đợi Pha B ảnh hiện tại), B-worker chạy tuần tự đúng thứ tự đọc (bắt buộc — `dialogueWindow`/`registerRenderedRegion`/`gpt_config` dùng chung instance translator đều phụ thuộc thứ tự, xem comment `runPhaseB`). Chặn ở 2 vì kết quả Pha A mang theo ảnh nền đã inpaint (vài MB/trang) — không thể để Pha A chạy xa tuỳ ý.

**Vì sao chọn `translator:'none'` cho Pha A (không phải build endpoint detect-riêng):** tái dùng đúng endpoint cũ, không cần thay đổi backend (Task 2 của kế hoạch chỉ đổi config phía client). Cái giá: backend có 1 bộ lọc "Translation identical to original" (`manga_translator.py`) chỉ chạy khi `translator != none` — Pha A miễn nhiễm hoàn toàn, nên Pha A trả về NHIỀU vùng hơn đường cũ trên các trang có tên riêng/SFX/watermark mà GPT trả lại y nguyên (đo thực nghiệm 2026-09-06: 9/12 trang lệch, y hệt tập hợp con — không phải nhiễu detection). `content.js` đã có sẵn (từ 2026-08-11, commit `bbbc870`, KHÔNG liên quan gì tới nhánh này) đúng 1 bộ lọc hẹp hơn để xử lý đúng ca này — chỉ bỏ `dst==src` khi `src` KHÔNG phải chữ Latin (SFX CJK giữ nguyên), còn chữ Latin (tên riêng, SFX tiếng Anh, watermark) thì GIỮ LẠI và vẽ đè lên nền đã inpaint — Kết luận ban đầu cho rằng đây là Pha A "sửa" một khoảng hở — **ĐÃ BỊ BÁC BỎ ở review toàn nhánh 2026-09-05**. (Sau khi sửa, đo lại đầu-cuối 2026-09-06: tập vùng ĐƯỢC VẼ khớp đường cũ **8/8 trang**, cả MangaDex lẫn hitomi — Pha A vẫn trả nhiều vùng hơn (7 vs 5, 8 vs 7) nhưng `region-filter.js` loại đúng phần thừa.) Bộ lọc backend chạy TRƯỚC bước mask+inpaint, nên ở đường cũ vùng bị loại KHÔNG hề bị inpaint và art gốc còn nguyên; ở đường mới vùng đó bị inpaint (xoá nét gốc) rồi vẽ lại đúng chuỗi ấy bằng font overlay — mất mát thuần tuý, vì `dst == src` nghĩa là overlay không mang thêm thông tin nào người đọc chưa đọc được trên art. `gpt_config-vi.yaml` còn chủ động dặn model trả nguyên văn tên riêng Latin kèm ghi chú rằng client sẽ không vẽ đè các đoạn đó — tức đây là hành vi có chủ đích đang bị phá. **Đã sửa 2026-09-05:** bỏ điều kiện Latin, tách thành `extension/content-script/region-filter.js` (`motShouldRenderRegion`) để test được — chính vì trước đây nằm trong IIFE không test được mà lỗi lọt qua ba lượt review. Chi tiết đầy đủ (kèm bằng chứng tái hiện qua `curl`): `.superpowers/sdd/2026-09-05-split-phase-pipeline/task-6-report.md`.

**Đã đo (`tools/measure-pipeline.js`, qua backend thật, không qua trình duyệt):** thời gian giữ khoá executor/trang giảm từ trung bình 10895ms (đường cũ) xuống 4273ms (chỉ Pha A) — xem mục 8 ("Eager mode dồn cả chương..."). **Chưa đo — đọc kỹ trước khi trích số:** (a) ~~Pha B chưa từng được đo trực tiếp~~ — **ĐÃ ĐO TRỰC TIẾP 2026-09-06**, chạy trọn luồng hai pha (Pha A → `/translate/texts` → `region-filter.js` thật) trên 8 trang thật (5 MangaDex scan tiếng Anh + 3 hitomi AVIF/CJK): **Pha A trung vị 4966ms, Pha B trung vị 2430ms, đường cũ trung vị 6666ms**. Kiểm chứng mô hình `cũ ≈ A + B`: trang 13 là 3768+2430=6198 so với 6406 đo được; trang h02 là 5496+3432=8928 so với 9011. **Pha A LỚN HƠN Pha B ở CẢ 8/8 trang**, nên `max(A,B)` luôn bằng A và thông lượng ổn định chỉ cải thiện **6666→4966ms, tức ~1,34×** — KHÔNG phải ~2× như kế hoạch dự đoán. Kế hoạch ước A≈3,4s/B≈4,0s; thực tế ngược lại (A≈5,0s vì có inpaint thật, B≈2,4s vì bỏ tầng kiểm tra hậu-dịch — xem (c)). **Lợi ích thật nằm ở đuôi phân bố, không ở trung vị:** khi GPT gặp bão retry (đo được 27–30s trên 1 trang), đường cũ giữ khoá GPU suốt thời gian đó và chặn mọi trang khác; đường mới đẩy nó sang Pha B, ngoài khoá. (b) Độ trễ per-trang thật trong trình duyệt và việc hàng đợi cả chương có ngắn lại tương ứng hay không — cần browser thật. (c) **Pha B bỏ qua `enable_post_translation_check` (mặc định `True`)**: mất kiểm tra lặp/ảo giác kèm retry theo vùng, và kiểm tra tỉ lệ ngôn ngữ đích cấp trang kèm tối đa 3 lần retry cả lô; retry nội bộ của translator vẫn còn. Hệ quả: GPT lặp một cụm từ hoặc trả lời sai ngôn ngữ sẽ được vẽ nguyên như vậy thay vì bị chạy lại. (d) Con số 4273ms **không bao gồm** probe ghép-biên (~2,5s/trang khi có ảnh kế tiếp) và, khi có bong bóng vắt biên thật, một lượt gọi GPT crop vẫn chạy TRONG khoá executor.
...
- **Eager mode dồn cả chương vào 1 hàng đợi tuần tự** — ảnh cuối chương chờ 7–16 phút (mục 5.10). **Giảm nhẹ (không triệt để) bằng split-phase pipeline, đo 2026-09-06 (mục 5.8.1):** thời gian MỘT trang giữ khoá executor GPU (chính là tài nguyên làm nghẽn hàng đợi ở `CONCURRENCY:1`) giảm từ trung bình 10895ms/median 6874ms (đường cũ: detect+OCR+dịch GPT+inpaint trong 1 lần gọi) xuống trung bình 4273ms/median 4157ms (chỉ Pha A: detect+OCR+inpaint, không gọi GPT) — 1,3×–2,1× trên trang thường, **5,76× trên 1 trang bị GPT retry-storm** (29s → 5s, đo lặp lại 4/4 lần, không phải may rủi). Đo trực tiếp qua backend thật (`tools/measure-pipeline.js`), KHÔNG qua trình duyệt — xem `task-6-report.md`. **CHƯA xác nhận:** số giây per-trang người đọc thực sự thấy, và việc hàng đợi cả chương có thực sự ngắn lại 7–16 phút → còn lại bao nhiêu (cần 1 phiên browser thật với eager mode + chương nhiều trang, chưa làm).
...
- **"Khởi động nguội làm ngôi xưng dao động" — ĐÃ ĐO KỸ VÀ BÁC BỎ (2026-08-26). Đừng xây lại cơ chế nào cho nó.** Nghi vấn ban đầu: trang đầu chương chưa có cửa sổ nên lựa chọn đầu tiên khoá cả chương, và đo 3 lần thấy 1 lần lệch (~17%). Đo lại **16 lần mỗi phía** bằng một dòng dò duy nhất xác định rõ người nói/người nghe (Annetta → Erkin, có chữ "YOU" nên buộc chọn đại từ): **0/16 sai ở cả hai phía**. Con số 17% kia là **lỗi thước đo**: nó gom mọi dòng có nhắc `ERKIN`/`ANNETTA` thành "cặp bạn bè", trong đó có cả các dòng Annetta nói với **ông Jenkins** — chỗ dùng `ông` là ĐÚNG. Dao động thật chỉ là giữa `anh` và `cậu`, đều hợp lệ cho bạn bè.
  Đã thử luôn phương án **gắn nhãn tên nhân vật vào cửa sổ** (tên riêng không được dịch nên xuất hiện nguyên văn ở cả `src` lẫn `dst`): 0/16 sai — **không cải thiện gì đo được**, chỉ dịch phân bố từ `anh` sang `cậu`. Không đáng xây.
  **Bài học về cách đo:** đếm gộp đại từ cả chương KHÔNG đáng tin — nó lẫn các lần đổi nhân vật hợp lệ vào lỗi. Phải khoanh vào một cặp nhân vật, một dòng cụ thể. Trong phiên này thước đo gộp đã đánh lừa ba lần liên tiếp.
