...
- `translateImage(blob, detectOnly=false)` — dựng đúng `config` (detector/translator/render/inpainter theo `CFG`), gửi message `TRANSLATE`. `detectOnly=true` ép `translator:'none'`, bỏ `gpt_config`/`inpainter` khỏi request (mục 2.6) — dùng cho gate ghép-biên (mục 5.7).
