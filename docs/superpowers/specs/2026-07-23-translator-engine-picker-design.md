...
- **Cách người dùng CẬP NHẬT key sau khi đã cài đặt** (không chỉ nhập lần đầu): nhu cầu thật, được nêu ra khi brainstorm spec này, nhưng thuộc phạm vi installer (`setup.bat`, đang tạm dừng — xem memory `manga-distribution-installer`), không phải phạm vi spec này. Khi re-scope installer, cần thêm 1 script/shortcut **chạy lại được bất cứ lúc nào** (không phải chỉ chạy 1 lần lúc cài đặt) để nhập/sửa cả 3 key (OpenAI/Gemini/DeepL) rồi ghi đè `.env` + khởi động lại container — không bắt người dùng chạy lại toàn bộ luồng cài đặt chỉ để đổi 1 key.
