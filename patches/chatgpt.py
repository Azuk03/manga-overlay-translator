    ...
    async def _translate_batch(  
        ...
        for attempt in range(max_attempts):  
            try:  
                ...
                #
                # [VA CUA DU AN 2026-08-26] Ban goc quet TUNG DONG bang
                #   re.match(r'^<\|(\d+)\|>(.*)', line)
                # nen BAT BUOC marker phai nam dau dong. Nhung buoc TRICH ngay o tren
                # (re.split(r'<\|\d+\|>', response_text)) lai tim marker o BAT KY dau.
                # Hai cach hieu lech nhau ngay trong cung mot ham: mot response ma buoc
                # trich xu ly duoc hoan hao van bi buoc kiem tra bac bo, chi vi model
                # khong xuong dong truoc marker.
                #
                # Hau qua do duoc tren log that (phien webtoon 2026-08-26, 180 luot goi):
                # 3/79 batch bao "Found indices count (1) does not match expected count (2)"
                # du response co du ca <|1|> lan <|2|>, roi thu lai 3 lan - moi lan GPT
                # tra ve mot ban dich KHAC (GIAT MINH / Rung minh / RUT RE). Ton tien GPT
                # gap 3 va ban thang cuoc la ban ngau nhien, nen ban dich luc VIET HOA
                # luc viet thuong ngay trong cung mot chuong.
                #
                # Sua: dung CHUNG mot cach hieu voi buoc trich. Moi phep kiem tra khac
                # (trung index, index ngoai pham vi, thieu index) giu nguyen y het ban goc.
                is_valid_format = True
                if not merged_single_query:
                    if not response_text or not response_text.strip():
                        self.logger.warning(
                            f"[Attempt {attempt+1}/{max_attempts}] Received empty response for non-empty batch. Retrying..."
                        )
                        is_valid_format = False
                    else:
                        expected_indices = set(range(1, len(batch_queries) + 1))
                        found_indices = set()
                        for current_index in find_marker_indices(response_text):
                            if current_index in expected_indices:
                                if current_index in found_indices:
                                    self.logger.warning(
                                        f"[Attempt {attempt+1}/{max_attempts}] Duplicate index {current_index} detected. Retrying..."
                                    )
                                    is_valid_format = False
                                    break
                                ...
                                self.logger.warning(
                                    f"[Attempt {attempt+1}/{max_attempts}] Invalid index {current_index} found (expected 1-{len(batch_queries)}). Retrying..."
                                )
                                is_valid_format = False
                                break
                    ...
                    if is_valid_format:
                        if len(found_indices) != len(batch_queries):
                            ...
                            is_valid_format = False
                        elif found_indices != expected_indices:
                            self.logger.warning(
