...
def to_translation(ctx: Context) -> TranslationResponse:
    ...
    for i, blk in enumerate(text_regions):
        ...
        trans = {key: value[i] for key, value in translations.items()} if translations else {}
        trans["src"] = text_region.text
        translated = getattr(text_region, "translation", "")
        if translated:
            trans["dst"] = translated
